...
#ifdef ENABLE_NAT46
		if (unlikely((tuple->addr.p1 == 0) && (tuple->addr.p2 == 0) &&
		    (tuple->addr.p3 == 0xFFFF0000))) {
			union v6addr dp = NAT46_DST_PREFIX;

			ret = ipv6_to_ipv4(skb, 14, &dp, htonl(LXC_IPV4));
			if (IS_ERR(ret))
				return ret;
...
to_host:
	if (1) {
		...
		if (skb->cb[CB_NAT46_STATE] == NAT46) {
			...
			cilium_trace(skb, DBG_TO_HOST, 2, 2);
			ret = ipv4_to_ipv6(skb, ip4, 14, &sp, &dp, 0);
			if (IS_ERR(ret))
				return ret;
		}
#endif
...
static inline int __inline__ ipv4_policy(struct __sk_buff *skb, int ifindex, __u32 src_label)
{
	...
	if (skb->cb[CB_NAT46_STATE] == NAT46) {
		...
		ret = ipv4_to_ipv6(skb, ip4, 14, &sp, &dp, 1);
		if (IS_ERR(ret))
			return ret;
