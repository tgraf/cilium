...
func parseServiceKey(address string) *types.L3n4AddrID {
	...
	l3n4Addr, err := types.NewL3n4AddrID(types.TCP, frontend.IP, uint16(frontend.Port), 0)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Unable to parse frontend address: %s\n", err)
		os.Exit(1)
	}
...
func cliUpdateService(ctx *cli.Context) {
	fe := parseServiceKey(ctx.String("frontend"))
	backends := []types.L3n4Addr{}
	fe.ID = types.ServiceID(ctx.Int("id"))
