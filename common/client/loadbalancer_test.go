...
func (s *CiliumNetClientSuite) TestSVCGetOK(c *C) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		err := json.NewEncoder(w).Encode(wantLBSVC.BES)
		c.Assert(err, Equals, nil)
	}))
	defer server.Close()
	...
	besReceived, err := cli.SVCGet(*fe)

	c.Assert(err, IsNil)
	c.Assert(besReceived, DeepEquals, wantLBSVC.BES)
}
...
func (s *CiliumNetClientSuite) TestSVCGetFail(c *C) {
	...
	feReceived, err := cli.SVCGet(*fe)

	c.Assert(strings.Contains(err.Error(), "daemon didn't complete your request"), Equals, true)
	var nilFe []types.L3n4Addr
	c.Assert(feReceived, DeepEquals, nilFe)
}
