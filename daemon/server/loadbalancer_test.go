...
func (s *DaemonSuite) TestSVCGetOK(c *C) {
	...
	s.d.OnSVCGetBySHA256Sum = func(feL3n4SHA256Sum string) ([]types.L3n4Addr, error) {
		c.Assert(feL3n4SHA256Sum, Equals, feL3n4SHA256Want)
		return bes, nil
	}

	besReceived, err := s.c.SVCGet(*feWant)
	c.Assert(err, IsNil)
	c.Assert(besReceived, DeepEquals, bes)
}
...
func (s *DaemonSuite) TestSVCGetBySHA256SumOK(c *C) {
	...
	besReceived, err := s.c.SVCGetBySHA256Sum(feL3n4SHA256Want)
	c.Assert(err, IsNil)
	c.Assert(besReceived, DeepEquals, bes)
}
