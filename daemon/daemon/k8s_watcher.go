...
func (d *Daemon) delK8sSVCs(svc types.K8sServiceNamespace, svcInfo *types.K8sServiceInfo, se *types.K8sServiceEndpoint) {
	...
	for _, svcPort := range svcInfo.Ports {
		...
		fe, err := types.NewL3n4AddrID(svcPort.Protocol, svcInfo.FEIP, svcPort.Port, 0)
		if err != nil {
		...
		if err := d.RevNATDelete(fe.ID); err != nil {
			log.Warningf("Error deleting reverse NAT %+v, %s", fe.ID, err)
		} else {
